"""Provides device actions for Livemasjid."""
...
ACTION_TYPES = {"start_stream", "stop_stream"}
...
async def async_get_actions(hass: HomeAssistant, device_id: str) -> list[dict]:
    """List device actions for Livemasjid devices."""
    ...
    for entry in entity_registry.async_entries_for_device(registry, device_id):
        ...
        actions.append({**base_action, CONF_TYPE: "start_stream"})
        actions.append({**base_action, CONF_TYPE: "stop_stream"})
...
async def async_call_action_from_config(
    ...
    if config[CONF_TYPE] == "start_stream":
        service = SERVICE_TURN_ON
    elif config[CONF_TYPE] == "stop_stream":
